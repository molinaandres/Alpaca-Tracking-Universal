...
struct EditCredentialsView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            print("🔍 EditCredentialsView appeared. Accounts count: \(accountManager.realAccounts.count)")
            ...
            if accountManager.realAccounts.isEmpty && authService.isAuthenticated {
                print("🔄 No accounts found, attempting to load...")
    ...
    private func handleUpdateCredentials() {
        ...
        print("🔍 handleUpdateCredentials called. Accounts count: \(accountManager.realAccounts.count)")
        guard let account = accountManager.realAccounts.first else {
            print("❌ No account found in realAccounts")
            errorMessage = "No account found. Please add an account first."
            isLoading = false
            // Intentar cargar cuentas si estamos autenticados
